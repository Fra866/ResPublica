	...
	if menu_main:
		...
		if current_menu == party_menu:
			if len(voter_list):
				...
				# Had to put it here because the 'index' in the container
				# keeps reverting to 0 at the end of this if section
				
				if (Input.is_action_just_pressed("ui_right") && voter_index < len(party_menu.container.get_items()) - 1):
					voter_index += 1
				if (Input.is_action_just_pressed("ui_left") && voter_index > 0):
					voter_index -= 1
				
				party_menu.container.index = voter_index
				party_menu.container.current_el = party_menu.container.get_i(voter_index)
				
				party_menu.container.selector.rect_position = Vector2(32 * (voter_index % 4) + 5, 40 * (voter_index / 4) + 18)
...
func new_battleslog(element, i: int, period: int):
	...
	print(slogan_menu.battle_menu.get_children())
	var new_slog_instance = slogan_menu.battle_menu.get_child(period+1).new_item(pos)
	
	new_slog_instance.res = element.res
	new_slog_instance.visible = true
	slogan_menu.battle_menu.get_child(period+1).add(new_slog_instance)
	
	slogan_menu.battle_menu.get_child(period+1).shows(true)
	var a000 = slogan_menu.battle_menu.get_child(period+1).get_items()
